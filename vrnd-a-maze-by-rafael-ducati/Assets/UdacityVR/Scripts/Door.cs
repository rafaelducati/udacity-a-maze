...
public class Door : MonoBehaviour {
	...
	void Start () {
		...
		leftDoorEndRotation = leftDoorStartRotation * Quaternion.Euler(0, 0, 100);
		...
		rightDoorEndRotation = rightDoorStartRotation * Quaternion.Euler(0, 0, -100);
	}
	...
	public void OnDoorClicked () {
		...
		Debug.Log("foundKey restaurada: " + foundKey);
		Debug.Log("testMsg restaurada: " + testMsg);
		if(locked){
			audioSource.PlayOneShot(doorLockedClick);
			Debug.Log("You must find the key at the maze.");
			
		} else {
			opening = true;
			audioSource.PlayOneShot(doorOpenClick);
